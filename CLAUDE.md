...
- Translation: use `com.hbm.util.i18n.I18nUtil` (`resolveKey`, `resolveKeyArray`, `format`), never `net.minecraft.client.resources.I18n` (fork rule, inherited from upstream; 343 legacy direct calls remain). On the dedicated server `I18nUtil` returns "I18N CALL SERVERSIDE - GREAT JOB"; server text uses `ChatBuilder`/`ChatComponentTranslation`.
...
- Fork policy: DBS is a personal fork. `CONTRIBUTING.md` was removed in `ea346f74` and no upstream pull requests are planned, so upstream's rules (no refactors, no new libraries, no AI-written code) do not apply. Our own rules: `I18nUtil` over `I18n`; no `changelog` edits (merge conflicts); minimal diffs in upstream files by default, refactors as deliberate batches (`docs/fix-roadmap.md`, Phase 8); test on client and dedicated server. OC/NEI/AE2/CoFH are the existing compile dependencies; new soft dependencies stay behind `Loader.isModLoaded` / `@Optional`.
