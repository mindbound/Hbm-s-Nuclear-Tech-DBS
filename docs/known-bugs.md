...
Tracker for defects found by the 2026-09-24 code survey (19 subsystem readers, each adversarially verified) and re-verified item by item against the tree at `22c9c89c`. Line numbers are from that snapshot; class and method names are the stable anchors. Everything here is upstream (HbmMods / NTM: Space) code; the fork has not changed any of it yet.

How to use: pick from "Fix in the fork" first; each item there is safe to change without altering balance or saves. "Needs a decision" items change something players can notice; decide per item and record it. Update the **Status** field when acting (`open`, `fixed <commit>`, `wontfix <why>`, `reported upstream`). Fixes go through the fork-owned package or minimal, marked lines per `CLAUDE.md`; every fix is built and tested locally before merging.
...
|---|---|---|---|---|---|---|---|
| Fix in the fork | 13 | 4 | 54 | 36 | 10 | 15 | 132 |
| Needs a decision | 7 | 2 | 22 | 23 | 9 | 18 | 81 |
| Leave | 8 | 0 | 19 | 27 | 6 | 46 | 106 |
| Upstream-only | 1 | 0 | 5 | 1 | 1 | 3 | 11 |
...
|---|---|---|---|---|---|
| B-002 | crash | core | `src/main/java/com/hbm/potion/HbmPotion.java:86-99` | HbmPotion.registerPotion sizes potionTypes to max(256, id) and swallows errors | open |
| B-037 | crash | blocks-generic | `src/main/java/com/hbm/blocks/fluid/CoriumFluid.java:23-29` | CoriumFluid returns icons of never-instantiated CoriumBlock (null) | open |
| B-066 | crash | te-special | `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityRBMKControlAuto.java:117-127` | TileEntityRBMKControlAuto: null function breaks serialize/deserialize symmetry | open |
| B-067 | crash | te-special | `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityRBMKControlAuto.java:134-135` | TileEntityRBMKControlAuto.receiveControl mods function index by RBMKColor count | open |
| B-093 | crash | networks | `src/main/java/com/hbm/tileentity/network/TileEntityPylonBase.java:80-82` | TileEntityPylonBase.addConnection dereferences a possibly null PowerNode | open |
| B-094 | crash | networks | `src/main/java/com/hbm/uninos/UniNodespace.java:52-56` | UniNodespace.destroyNode(World, GenNode) NPEs when the world has no node map | open |
| B-195 | crash | hazards | `src/main/java/com/hbm/saveddata/SatelliteSavedData.java:48-49` | SatelliteSavedData.readFromNBT NPEs on an unknown satellite id | open |
| B-218 | crash | space | `src/main/java/com/hbm/world/feature/BedrockOre.java:56-60` | BedrockOre.generateAuto NPEs for a Body without a bedrock table | open |
| B-265 | crash | net-util | `src/main/java/com/hbm/packet/toserver/KeybindPacket.java:42` | KeybindPacket.Handler indexes EnumKeybind.values() with an unchecked client int | open |
| B-267 | crash | net-util | `src/main/java/com/hbm/util/BufferUtil.java:104,114` | BufferUtil.writeNBT stores the compressed length as a short | open |
| B-306 | crash | oc | `src/main/java/com/hbm/handler/CompatHandler.java:196-223` | CompatHandler.init adds a recipe for a floppy whose item may be null | open |
| B-307 | crash | oc | `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityRBMKControlManual.java:161-162` | RBMKControlManual.getColor NPEs when no colour is set | open |
| B-308 | crash | oc | `src/main/java/com/hbm/tileentity/turret/TileEntityTurretBaseArtillery.java:86-88` | TurretBaseArtillery.getCurrentTarget throws when target queue is empty | open |
| B-003 | save data | core | `src/main/java/com/hbm/extprop/HbmLivingProps.java:496-498` | ContaminationEffect.load reads time/maxTime/ignoreArmor from the parent compound | open |
| B-055 | save data | te-machine | `src/main/java/com/hbm/inventory/fluid/tank/FluidTank.java:264-268` | FluidTank.readFromNBT zeroes fill when `<key>_max` is absent | open |
| B-068 | save data | te-special | `src/main/java/com/hbm/tileentity/machine/TileEntityZirnoxDestroyed.java:30-36` | TileEntityZirnoxDestroyed NBT key mismatch: reads "fire", writes "onFire" | open |
| B-176 | save data | entities | `src/main/java/com/hbm/entity/EntityMappings.java:82-86` | Duplicate entity name entity_cloud_rainbow for two classes | open |
| B-004 | gameplay | core | `src/main/java/com/hbm/main/MainRegistry.java:367-377` | ForgeChunkManager callback returns after the first NORMAL ticket | open |
| B-019 | gameplay | api | `src/main/java/api/hbm/energymk2/IEnergyReceiverMK2.java:70-82` | IEnergyReceiverMK2.tryUnsubscribe uses createNode() instead of getNode (no-op) | open |
| B-024 | gameplay | api | `src/main/java/com/hbm/tileentity/machine/TileEntityXenonThruster.java:59-61` | TileEntityXenonThruster sets hasRegistered even when not facing prograde | open |
| B-040 | gameplay | blocks-generic | `src/main/java/com/hbm/blocks/fluid/GenericFiniteFluid.java:19-45` | GenericFiniteFluid static icons shared by concrete_liquid and corium_block | open |
| B-041 | gameplay | blocks-generic | `src/main/java/com/hbm/blocks/fluid/GenericFluidBlock.java:68-70` | GenericFluidBlock damage only applied for sulfuric_acid_block | open |
| B-050 | gameplay | blocks-machine | `src/main/java/com/hbm/blocks/BlockDummyable.java:757-760` | Placement preview rotates with `facing` while the footprint uses getDirModified | open |
| B-051 | gameplay | blocks-machine | `src/main/java/com/hbm/blocks/machine/MachineCoker.java:34` | 14 blocks pass the clicked side as GUI id to standardOpenBehavior | open |
| B-056 | gameplay | te-machine | `src/main/java/com/hbm/inventory/UpgradeManagerNT.java:57-66` | UpgradeManagerNT.mutexType never reset; re-inserted mutex upgrade ignored | open |
| B-072 | gameplay | te-special | `src/main/java/com/hbm/tileentity/bomb/TileEntityCompactLauncher.java:291` | sendCommandEntity passes posX for both x and z in two launchers | open |
| B-073 | gameplay | te-special | `src/main/java/com/hbm/tileentity/bomb/TileEntityLaunchPadLarge.java:99` | TileEntityLaunchPadLarge tests `erector == 1F` inside the lift-extension branch | open |
| B-074 | gameplay | te-special | `src/main/java/com/hbm/tileentity/bomb/TileEntityLaunchPadRocket.java:627-630` | TileEntityLaunchPadRocket.getDestination OC callback has inverted hasDrive() | open |
| B-078 | gameplay | te-special | `src/main/java/com/hbm/tileentity/machine/TileEntityReactorResearch.java:119-124` | Research reactor: canExtractItem uses fuelMap.containsValue(stack), always false | open |
| B-079 | gameplay | te-special | `src/main/java/com/hbm/tileentity/machine/TileEntityReactorResearch.java:89` | Research reactor: isItemValidForSlot only accepts slot 0 (`i < 12 && i <= 0`) | open |
| B-080 | gameplay | te-special | `src/main/java/com/hbm/tileentity/machine/rbmk/RBMKDials.java:261-266` | RBMKDials.getSurgeMod client fallback reads the passive-cooling dial | open |
| B-110 | gameplay | items | `src/main/java/com/hbm/main/ModEventHandlerClient.java:791` | Armor-mod tooltip loop stops at 8, hiding the battery slot mod | open |
| B-120 | gameplay | weapons | `src/main/java/com/hbm/items/weapon/sedna/factory/Lego.java:128-129` | LAMBDA_STANDARD_CLICK_SECONDARY reads mode of config 0, writes ctx.configIndex | open |
| B-125 | gameplay | weapons | `src/main/java/com/hbm/items/weapon/sedna/mods/XWeaponModManager.java:308` | Weapon-mod install keys the gun with a non-singular ComparableStack | open |
| B-137 | gameplay | recipes | `src/main/java/com/hbm/inventory/FluidContainerRegistry.java:96-97` | Disperser/gland containers registered by loop index instead of fluid id | open |
| B-155 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerBarrel.java:55-56` | ContainerBarrel tile-to-player merge skips player slot 6 | open |
| B-156 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerElectrolyserMetal.java:57-58` | ContainerElectrolyserMetal shift-click bounds off by one | open |
| B-157 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerMachineCryoDistill.java:61-71` | ContainerMachineCryoDistill merges into its own tile slots | open |
| B-158 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerMachineCyclotron.java:66-67` | ContainerMachineCyclotron shift-click bounds off by four | open |
| B-159 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerMachineDiesel.java:45-51` | ContainerMachineDiesel shift-click bounds off by one | open |
| B-160 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerMachineKeyForge.java:54-60` | ContainerMachineKeyForge / SatLinker route tile slots 1-2 into slot 0 | open |
| B-161 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerMachineMilkReformer.java:61-68` | ContainerMachineMilkReformer bounds off by two and empty battery range | open |
| B-162 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerMachineOilWell.java:55-62` | ContainerMachineOilWell shift-click off by one; upgrade range hits player slot | open |
| B-163 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerMachineReactorBreeding.java:46-47` | ContainerMachineReactorBreeding shift-click off-by-one (index <= 2 for 2 slots) | open |
| B-164 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerMachineVacuumDistill.java:72-73` | ContainerMachineVacuumDistill treats tile slot 11 as a player slot | open |
| B-165 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerNukeAntimatter.java:50-51` | ContainerNukeAntimatter: slots 3-4 not shift-clickable, player->bomb never works | open |
| B-166 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerNukeFstbmb.java:47-48` | ContainerNukeFstbmb treats player slot 2 as a bomb slot | open |
| B-167 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerPADetector.java:55-56` | ContainerPADetector / ContainerPASource shift-click bounds off by one | open |
| B-168 | gameplay | gui | `src/main/java/com/hbm/inventory/container/ContainerReactorResearch.java:56-57` | ContainerReactorResearch shift-click off-by-one (index <= 12 for 12 slots) | open |
| B-178 | gameplay | entities | `src/main/java/com/hbm/entity/EntityMappings.java:260-270` | EntityMappings.addSpawn appends a duplicate after adjusting an entry | open |
| B-179 | gameplay | entities | `src/main/java/com/hbm/entity/effect/EntityFalloutRain.java:38-42` | EntityFalloutRain(World, int maxAge) ignores maxAge and frustum flag | open |
| B-197 | gameplay | hazards | `src/main/java/com/hbm/handler/microblocks/MicroBlocksCompatHandler.java:78` | MicroBlocksCompatHandler skips metadata 15 | open |
| B-198 | gameplay | hazards | `src/main/java/com/hbm/handler/pollution/PollutionHandler.java:369-387` | PollutionHandler.decorateMob heals mobs on every LivingSpawnEvent subtype | open |
| B-199 | gameplay | hazards | `src/main/java/com/hbm/handler/radiation/ChunkRadiationHandlerPRISM.java:331-371` | PRISM SubChunk uses cX for the Z chunk origin | open |
| B-201 | gameplay | hazards | `src/main/java/com/hbm/saveddata/satellites/SatelliteMiner.java:56-58` | SatelliteMiner.getCargoForItem looks up an Item-keyed map with a ComparableStack | open |
| B-221 | gameplay | space | `src/main/java/com/hbm/dim/WorldProviderCelestial.java:723-725` | Sleeping on a planet resets local time to 0 instead of next morning | open |
| B-222 | gameplay | space | `src/main/java/com/hbm/dim/duna/biome/BiomeGenDunaPlains.java:76` | Seventeen celestial biomes use Math.random() in genTerrainBlocks | open |
| B-223 | gameplay | space | `src/main/java/com/hbm/dim/thatmo/WorldProviderThatmo.java:29-31` | WorldProviderThatmo passes dimensionId as rainfall and rebuilds its biome | open |
| B-227 | gameplay | space | `src/main/java/com/hbm/world/gen/NTMWorldGenerator.java:283-304` | NTMWorldGenerator global hasPopulationEvent flag suppresses planet structures | open |
| B-253 | gameplay | render | `src/main/java/com/hbm/main/ModEventHandlerClient.java:354-371` | Hotbar animation expiry runs ungated in RenderGameOverlayEvent.Pre | open |
| B-269 | gameplay | net-util | `src/main/java/com/hbm/packet/toclient/HbmAnimationPacket.java:88` | HbmAnimationPacket and PlayerInformPacket handlers swallow every exception | open |
| B-270 | gameplay | net-util | `src/main/java/com/hbm/util/BobMathUtil.java:45,55` | BobMathUtil.max(float...)/max(double...) seed with MIN_VALUE (smallest positive) | open |
| B-271 | gameplay | net-util | `src/main/java/com/hbm/util/BobMathUtil.java:64` | BobMathUtil.safeClamp tests `val == Double.NaN` (always false) | open |
| B-273 | gameplay | net-util | `src/main/java/com/hbm/util/Tuple.java:236` | Tuple.Quintet.equals compares v against other.w | open |
| B-310 | gameplay | oc | `src/main/java/com/hbm/tileentity/machine/TileEntityMachineLargeTurbine.java:316-339` | LargeTurbine methods() lists getPower but invoke() has no case | open |
| B-312 | gameplay | oc | `src/main/java/com/hbm/tileentity/machine/TileEntityReactorResearch.java:487-517` | ReactorResearch setLevel callback missing from methods()/invoke() | open |
| B-313 | gameplay | oc | `src/main/java/com/hbm/tileentity/machine/TileEntityWatz.java:626-651` | TileEntityWatz methods() lists bogus 'getComponentName' | open |
| B-314 | gameplay | oc | `src/main/java/com/hbm/tileentity/network/TileEntityRadioTelex.java:300-314` | RadioTelex.setSendingText errors when fewer than five arguments are passed | open |
| B-315 | gameplay | oc | `src/main/java/com/hbm/tileentity/turret/TileEntityTurretArty.java:472-490` | TileEntityTurretArty.methods() omits inherited getPos | open |
| B-316 | gameplay | oc | `src/main/java/com/hbm/tileentity/turret/TileEntityTurretBaseArtillery.java:86-93` | TurretBaseArtillery/HIMARS callbacks unreachable through ports | open |
| B-317 | gameplay | oc | `src/main/resources/assets/hbm/disks/pwrangler/usr/bin/PWRangler.lua:207-256` | PWRangler.lua uses Lua 5.3 '//' and cannot run on Lua 5.2 / OC-LuaJIT | open |
| B-006 | silent misconfig | core | `src/main/java/com/hbm/config/FalloutConfigJSON.java:292` | FalloutConfigJSON reads matchesMaterial from the mustBeOpaque key | open |
| B-009 | silent misconfig | core | `src/main/java/com/hbm/handler/Identity.java:35` | Identity.init writes -1 instead of the new random id to the identity file | open |
| B-027 | silent misconfig | api | `src/main/java/api/hbm/block/IToolable.java:32-45` | ToolType.getType freezes its lookup map on first call, ignoring later register() | open |
| B-053 | silent misconfig | blocks-machine | `src/main/java/com/hbm/blocks/BlockDummyableBeam.java:40` | BlockDummyableBeam.findCore(World) is an overload, not an override | open |
| B-059 | silent misconfig | te-machine | `src/main/java/com/hbm/config/MachineDynConfig.java:41` | MachineDynConfig swallows newInstance() failures silently | open |
| B-060 | silent misconfig | te-machine | `src/main/java/com/hbm/tileentity/machine/TileEntityMachineCentrifuge.java:387-388` | TileEntityMachineCentrifuge.provideExtraInfo writes two types under B_ACTIVE | open |
| B-083 | silent misconfig | te-special | `src/main/java/com/hbm/util/GameRuleHelper.java:29-30` | GameRuleHelper.parseDouble/parseInt create a gamerule named by the value string | open |
| B-098 | silent misconfig | networks | `src/main/java/com/hbm/handler/threading/PacketThreading.java:37` | PacketThreading.init() is never called during mod init | open |
| B-099 | silent misconfig | networks | `src/main/java/com/hbm/handler/threading/PacketThreading.java:44-47` | PacketThreading.init() rejects max > core (validation inverted) | open |
| B-141 | silent misconfig | recipes | `src/main/java/com/hbm/inventory/fluid/Fluids.java:1085-1099` | Fluids.readTraits clears traits, and FT_Rocket cannot be instantiated | open |
| B-147 | silent misconfig | recipes | `src/main/java/com/hbm/inventory/recipes/loader/GenericRecipes.java:99-100` | GenericRecipes.register appends to recipeOrderedList before the duplicate check | open |
| B-150 | silent misconfig | recipes | `src/main/java/com/hbm/inventory/recipes/loader/SerializableRecipe.java:276` | readAStack drops metadata for NBT stacks that writeAStack emits | open |
| B-172 | silent misconfig | gui | `src/main/java/com/hbm/handler/nei/BreederRecipeHandler.java:70-120` | BreederRecipeHandler hard-codes the 'breeding' id instead of getRecipeID() | open |
| B-204 | silent misconfig | hazards | `src/main/java/com/hbm/handler/imc/IMCBlastFurnace.java:64` | IMCBlastFurnace reads input2 ore list with the wrong tag type | open |
| B-206 | silent misconfig | hazards | `src/main/java/com/hbm/hazard/HazardSystem.java:62-63` | HazardSystem.register(ItemStack) does not makeSingular the key | open |
| B-229 | silent misconfig | space | `src/main/java/com/hbm/config/StructureConfig.java:14-78` | StructureConfig.structureMaxChunks field default 12 vs config default 16 | open |
| B-230 | silent misconfig | space | `src/main/java/com/hbm/dim/CelestialBody.java:258-272` | CelestialBody.getTraits returns the live saved trait map despite its comment | open |
| B-236 | silent misconfig | space | `src/main/java/com/hbm/dim/SolarSystemWorldSavedData.java:63-67` | SolarSystemWorldSavedData.readFromNBT silently drops traits that fail to load | open |
| B-237 | silent misconfig | space | `src/main/java/com/hbm/world/ModBiomes.java:13-17` | ModBiomes double-registers Duna biome dictionary types and tags dunaHills twice | open |
| B-256 | silent misconfig | render | `src/main/java/com/hbm/main/ClientProxy.java:2183-2206` | ClientProxy.getLoopedSound discards the pitch argument | open |
| B-257 | silent misconfig | render | `src/main/java/com/hbm/main/ResourceManager.java:645-1471` | ResourceManager holds 45 texture paths that do not exist on disk | open |
| B-258 | silent misconfig | render | `src/main/java/com/hbm/render/anim/AnimationLoader.java:68-77` | AnimationLoader parses rotmode but never stores it | open |
| B-275 | silent misconfig | net-util | `src/main/java/com/hbm/main/NetworkHandler.java:56-57` | Unregistered IMessage encodes as discriminator 0 and mis-decodes as TESiren | open |
| B-288 | silent misconfig | resources-build | `src/main/java/com/hbm/entity/missile/EntityRideableRocket.java:400` | Rocket 'hbm:entity.pipefail' sound has no sounds.json event | open |
| B-289 | silent misconfig | resources-build | `src/main/java/com/hbm/entity/mob/EntityHunterChopper.java:181` | EntityHunterChopper plays unknown sound 'hbm:weapon.osiprShoot' | open |
| B-290 | silent misconfig | resources-build | `src/main/java/com/hbm/items/armor/ItemModSensor.java:70` | ItemModSensor plays unknown sound 'hbm:weapon.follyAquired' | open |
| B-291 | silent misconfig | resources-build | `src/main/java/com/hbm/items/food/ItemLemon.java:114` | ItemLemon plays 'hbm:entity.vomit' but the event is player.vomit | open |
| B-292 | silent misconfig | resources-build | `src/main/resources/assets/hbm/lang/en_US.lang:1709-5422` | en_US.lang has 23 duplicate keys, two with conflicting values | open |
| B-294 | silent misconfig | resources-build | `src/main/resources/assets/hbm/manual/concepts/fluidhandling.json` | QMAW page fluidhandling.json never loads (no trigger, empty content) | open |
| B-295 | silent misconfig | resources-build | `src/main/resources/assets/hbm/manual/satellite/detector.json` | Satellite manual pages trigger on non-existent hbm:item.satellite | open |
| B-296 | silent misconfig | resources-build | `src/main/resources/assets/hbm/sounds.json:248` | sounds.json weapon.grenadeBounce lists missing grenadeBounce2.ogg | open |
| B-318 | silent misconfig | oc | `src/main/java/com/hbm/blocks/network/BlockOpenComputersCablePaintable.java:189-191` | OC cable @Optional.Interface names the wrong Colored interface | open |
| B-321 | silent misconfig | oc | `src/main/java/com/hbm/tileentity/bomb/TileEntityLaunchTable.java:53` | TileEntityLaunchTable implements OCComponent without direct SimpleComponent | open |
| B-323 | silent misconfig | oc | `src/main/java/com/hbm/tileentity/machine/TileEntityWatz.java:55` | TileEntityWatz implements OCComponent without direct SimpleComponent | open |
| B-326 | silent misconfig | oc | `src/main/java/com/hbm/tileentity/machine/storage/TileEntityMachineFluidTank.java:58` | Six TEs use lowercase modid 'opencomputers' in @Optional.Interface | open |
| B-327 | silent misconfig | oc | `src/main/java/com/hbm/tileentity/network/TileEntityRadioTorchBase.java:122` | Radio torch OC doc string misspells setChannel as 'setChannle' | open |
| B-028 | leak / perf | api | `src/main/java/api/hbm/energymk2/IBatteryItem.java:31-34` | IBatteryItem.emptyBattery copies the stack twice | open |
| B-054 | leak / perf | blocks-machine | `src/main/java/com/hbm/blocks/machine/BlockMachineBase.java:51-56` | BlockMachineBase.breakBlock skips super when TE is not ISidedInventory | open |
| B-061 | leak / perf | te-machine | `src/main/java/com/hbm/tileentity/TileEntityLoadedBase.java:36-39` | 25 onChunkUnload overrides skip super, so isLoaded never becomes false | open |
| B-100 | leak / perf | networks | `src/main/java/com/hbm/handler/threading/PacketThreading.java:97,123` | Threaded send recompiles every packet on the worker thread; reuse throws | open |
| B-101 | leak / perf | networks | `src/main/java/com/hbm/tileentity/machine/storage/TileEntityBatteryBase.java:65,102,111` | TileEntityBatteryBase node created at ports but looked up/destroyed at own xyz | open |
| B-104 | leak / perf | networks | `src/main/java/com/hbm/tileentity/network/RequestNetwork.java:43-48` | RequestNetwork.updateEntries 20-tick throttle is dead code (purges every tick) | open |
| B-107 | leak / perf | networks | `src/main/java/com/hbm/uninos/UniNodespace.java:27` | UniNodespace.worlds is keyed by World and never cleared on unload | open |
| B-212 | leak / perf | hazards | `src/main/java/com/hbm/handler/radiation/ChunkRadiationHandlerSimple.java:181` | Simple radiation backend unload removes by Chunk instead of ChunkCoordIntPair | open |
| B-238 | leak / perf | space | `src/main/java/com/hbm/dim/Ike/WorldGeneratorIke.java:67-82` | WorldGeneratorIke scans every block column of each chunk for pedestals | open |
| B-278 | leak / perf | net-util | `src/main/java/com/hbm/packet/toclient/BufPacket.java:57-67` | BufPacket/EntityBufPacket handlers leak the inbound buffer for non-receivers | open |
| B-034 | dead code | api | `src/main/java/api/hbm/fluidmk2/FluidNetMK2.java:49` | FluidNetMK2.transfered[] is allocated and reset but never written | open |
| B-085 | dead code | te-special | `src/main/java/com/hbm/tileentity/bomb/EntityAntimatter.java:1` | tileentity/bomb/EntityAntimatter.java is a 0-byte file | open |
| B-151 | dead code | recipes | `src/main/java/com/hbm/inventory/recipes/SILEXRecipes.java:51-58` | SILEXRecipes registers ingot_cm_mix twice | open |
| B-153 | dead code | recipes | `src/main/java/com/hbm/itempool/ItemPoolsSingle.java:116-130` | ItemPoolsSingle registers POOL_BLUEPRINTS twice | open |
| B-191 | dead code | entities | `src/main/java/com/hbm/entity/train/EntityRailCarBase.java:119-133` | EntityRailCarBase.interactFirst builds debug NBT that is never sent | open |
| B-214 | dead code | hazards | `src/main/java/com/hbm/handler/ThreeInts.java:46-48` | ThreeInts.compareTo violates the Comparable contract | open |
| B-217 | dead code | hazards | `src/main/java/com/hbm/main/ModEventHandler.java:1466-1474` | Legacy neutron-activation branch in ModEventHandler.onPlayerTick is unreachable | open |
| B-241 | dead code | space | `src/main/java/com/hbm/dim/dres/WorldGeneratorDres.java:77-80` | WorldGeneratorDres registers ore_lanthanium as a valid body twice | open |
| B-243 | dead code | space | `src/main/java/com/hbm/dim/trait/CBT_Compromised.java:3` | Empty top-level CBT_Compromised shadows the registered inner CBT_COMPROMISED | open |
| B-261 | dead code | render | `src/main/java/com/hbm/render/tileentity/RenderBombMultiLarge.java:15-22` | RenderBombMultiLarge unbound and references missing model/texture | open |
| B-262 | dead code | render | `src/main/java/com/hbm/render/tileentity/RenderVaultDoor.java:34-77` | Legacy render/tileentity/RenderVaultDoor is unbound and uses 11 missing textures | open |
| B-279 | dead code | net-util | `src/main/java/com/hbm/main/NetworkHandler.java:168-183` | NetworkHandler.sendToAllAround(ByteBuf, TargetPoint) is an unused trap overload | open |
| B-297 | dead code | resources-build | `src/main/java/com/hbm/main/StructureManager.java:165` | StructureManager.excavator references missing structures/excavator.nbt | open |
| B-300 | dead code | resources-build | `src/main/resources/assets/hbm/models/weapons/.obj` | Corrupt, nameless models/weapons/.obj ships in the jar | open |
| B-303 | dead code | resources-build | `src/main/resources/assets/hbm/sounds.json:393-395` | sounds.json fm.clap/fm.mug/fm.sample point at non-existent root files | open |

## Needs a decision (changes balance, saves or player-visible behaviour)

| ID | Severity | Area | Location | Title | Status |
|---|---|---|---|---|---|
| B-001 | crash | core | `src/main/java/com/hbm/handler/BlockMigrations.java:43` | BlockMigrations.doMigraion loops y upward from the height value (infinite loop) | open |
| B-049 | crash | blocks-machine | `src/main/java/com/hbm/blocks/BlockDummyable.java:174` | BlockDummyable.positions/internalPlayers: unsynchronized per-Block scratch state | open |
| B-134 | crash | recipes | `src/main/java/com/hbm/inventory/recipes/RadiolysisRecipes.java:54-57` | RadiolysisRecipes.registerRadiolysis throws when cracking recipes are empty | open |
| B-135 | crash | recipes | `src/main/java/com/hbm/inventory/recipes/loader/GenericRecipes.java:116-117` | GenericRecipes.readRecipe NPEs when user JSON omits duration or power | open |
| B-136 | crash | recipes | `src/main/java/com/hbm/inventory/recipes/loader/SerializableRecipe.java:126-160` | SerializableRecipe.initialize has no per-handler error isolation | open |
| B-193 | crash | hazards | `src/main/java/com/hbm/handler/atmosphere/AtmosphereBlob.java:171-175` | AtmosphereBlob flood fill reads world blocks from a pool thread | open |
| B-266 | crash | net-util | `src/main/java/com/hbm/util/ArmorUtil.java:235-250` | ArmorUtil static initializer throws AssertionError on unreflectable fiskheroes | open |
| B-069 | save data | te-special | `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityRBMKBase.java:305-309` | Static `diag` flag makes RBMK writeToNBT skip base/items during client HUD dump | open |
| B-177 | save data | entities | `src/main/java/com/hbm/entity/logic/EntityNukeExplosionMK5.java:144-151` | MK5 nuke explosion loses all state on save/reload | open |
| B-005 | gameplay | core | `src/main/java/com/hbm/main/ModEventHandler.java:1664-1670` | onPlayerClone resets NBT-only player props on every clone, not only on death | open |
| B-020 | gameplay | api | `src/main/java/api/hbm/energymk2/PowerNetMK2.java:128-145` | PowerNetMK2.sendPowerDiode lacks isBadLink, zero-demand skip and demand clamp | open |
| B-021 | gameplay | api | `src/main/java/api/hbm/energymk2/PowerNetMK2.java:85,149` | PowerNetMK2 priority loop subtracts the cumulative energyUsed every tier | open |
| B-022 | gameplay | api | `src/main/java/api/hbm/fluidmk2/FluidNetMK2.java:108` | FluidNetMK2.transferFluid subtracts cumulative received[p] per priority tier | open |
| B-023 | gameplay | api | `src/main/java/api/hbm/fluidmk2/IFluidStandardSenderMK2.java:48-57` | IFluidStandardSenderMK2.tryProvide direct push has no allowDirectProvision gate | open |
| B-070 | gameplay | te-special | `src/main/java/com/hbm/handler/neutron/RBMKNeutronHandler.java:286-290` | Blocked or partially blocked streams irradiate the origin column | open |
| B-076 | gameplay | te-special | `src/main/java/com/hbm/tileentity/deco/TileEntityTrappedBrick.java:77-78` | Trapped brick POISON_DART variant is selectable but unimplemented | open |
| B-077 | gameplay | te-special | `src/main/java/com/hbm/tileentity/machine/TileEntityMachineReactorBreeding.java:59` | Breeding reactor progress uses integer division of flux | open |
| B-095 | gameplay | networks | `src/main/java/com/hbm/handler/threading/PacketThreading.java:147,165-168` | packetThreadingErrorBypass skips the join entirely and drops the remaining queue | open |
| B-096 | gameplay | networks | `src/main/java/com/hbm/handler/threading/PacketThreading.java:150-160,193` | waitUntilThreadFinished drops queued packets after 50 ms and latches to sync | open |
| B-121 | gameplay | weapons | `src/main/java/com/hbm/items/weapon/sedna/mags/MagazineBelt.java:159-161` | MagazineBelt persists its type under an un-indexed 'magtype' NBT key | open |
| B-169 | gameplay | gui | `src/main/java/com/hbm/inventory/gui/GuiInfoContainer.java:362-371` | NEI drag-and-drop dead for raw containers with SlotPattern | open |
| B-170 | gameplay | gui | `src/main/java/com/hbm/main/NEIRegistry.java:14-16` | GTNH NEI: handler list built (and memoised) before recipes are initialised | open |
| B-182 | gameplay | entities | `src/main/java/com/hbm/entity/mob/glyphid/GlyphidStats.java:13-15` | Active glyphid stat set GLYPHID_STATS_NT is marked UNTESTED | open |
| B-183 | gameplay | entities | `src/main/java/com/hbm/explosion/ExplosionNukeRayBatched.java:89-98` | Batched nuke ray decays resistance over strength but cuts rays at length | open |
| B-200 | gameplay | hazards | `src/main/java/com/hbm/hazard/type/HazardTypeRadiation.java:34-36` | Neutron activation NBT is counted twice in radiation hazard level | open |
| B-224 | gameplay | space | `src/main/java/com/hbm/lib/HbmWorldGen.java:88-102` | HbmWorldGen rolls meteorites and crashed spaceships on every celestial dimension | open |
| B-225 | gameplay | space | `src/main/java/com/hbm/main/ModEventHandler.java:938-972` | updateWaterOpacity mutates Blocks.water opacity globally on every world tick | open |
| B-226 | gameplay | space | `src/main/java/com/hbm/world/feature/OreLayer3D.java:137` | OreLayer3D reuses cacheX for the z noise axis | open |
| B-272 | gameplay | net-util | `src/main/java/com/hbm/util/BufferUtil.java:116-117,158` | BufferUtil.readNBT returns a new empty compound for the null marker | open |
...
| B-052 | silent misconfig | blocks-machine | `src/main/java/com/hbm/blocks/BlockDummyable.java:71-80` | BlockDummyable.safeRem/overrideTileMeta statics toggled without try/finally | open |
| B-081 | silent misconfig | te-special | `src/main/java/com/hbm/tileentity/machine/TileEntityPWRController.java:678` | PWR ROR: value "rods" is 100 - rodLevel while setrods takes raw percent | open |
| B-113 | silent misconfig | items | `src/main/java/com/hbm/items/food/ItemLemon.java:40` | item.med_ipecac.desc key uses Cyrillic 'c' in code and en_US/uk_UA/zh_CN | open |
| B-145 | silent misconfig | recipes | `src/main/java/com/hbm/inventory/recipes/AtmosphereRecipes.java:70-73` | AtmosphereRecipes JSON read/write are no-ops | open |
| B-186 | silent misconfig | entities | `src/main/java/com/hbm/entity/logic/EntityNukeExplosionMK3.java:63` | BombConfig.limitExplosionLifespan only honoured by MK3 | open |
| B-187 | silent misconfig | entities | `src/main/java/com/hbm/main/ClientProxy.java:796` | EntitySiegeTunneler has a renderer but is never registered | open |
| B-202 | silent misconfig | hazards | `src/main/java/com/hbm/config/RadiationConfig.java:31-73` | RadiationConfig.disableNeutron defaults to true | open |
| B-209 | silent misconfig | hazards | `src/main/java/com/hbm/saveddata/satellites/XSatelliteRegistry.java:35-36` | SatelliteHorizons and SatelliteRailgun registered with colour 0,0,0 | open |
| B-231 | silent misconfig | space | `src/main/java/com/hbm/dim/CelestialBody.java:262-266` | getTraits fallback map exposes shared default trait instances to mutation | open |
| B-233 | silent misconfig | space | `src/main/java/com/hbm/dim/ChunkProviderCelestial.java:413-430` | Planet chunk providers never post PopulateChunkEvent | open |
| B-234 | silent misconfig | space | `src/main/java/com/hbm/dim/SolarSystem.java:275-321` | Thatmo dimension is registered but its body is unbound | open |
| B-286 | silent misconfig | resources-build | `README.md:48-49` | README maven coordinates omit the _H261 suffix build.gradle publishes | open |
| B-287 | silent misconfig | resources-build | `gradle.properties:19-82` | gradle.properties credits has mojibake and punctuation errors | open |
| B-293 | silent misconfig | resources-build | `src/main/resources/assets/hbm/lang/it_IT.lang` | it_IT/zh_CN/ru_RU lang files carry duplicate keys | open |
| B-319 | silent misconfig | oc | `src/main/java/com/hbm/tileentity/TileEntityProxyCombo.java:34-35` | TileEntityProxyCombo @Optional.Interface uses lowercase modid 'opencomputers' | open |
| B-320 | silent misconfig | oc | `src/main/java/com/hbm/tileentity/TileEntityProxyCombo.java:502-511` | ProxyCombo reports cached 'ntm_null' name before world load | open |
| B-322 | silent misconfig | oc | `src/main/java/com/hbm/tileentity/machine/TileEntityMachineIndustrialTurbine.java:260` | Component names reused with divergent APIs (ntm_turbine, ntm_energy_storage) | open |
| B-328 | silent misconfig | oc | `src/main/java/com/hbm/tileentity/network/TileEntityRadioTorchBase.java:146-151` | setCustomMapValues looks up Integer keys in an OC table map | open |
| B-029 | leak / perf | api | `src/main/java/api/hbm/energymk2/PowerNetMK2.java:31-32` | PowerNetMK2/FluidNetMK2.update return early on one-sided nets (stale entries) | open |
| B-030 | leak / perf | api | `src/main/java/api/hbm/tile/ILoadedTile.java:20,48` | ILoadedTile.TileAccessCache is a static map that is never evicted | open |
| B-103 | leak / perf | networks | `src/main/java/com/hbm/tileentity/network/RTTYSystem.java:64-72` | RTTYSystem injects the '2012-08-06' test channel into every world every tick | open |
| B-105 | leak / perf | networks | `src/main/java/com/hbm/tileentity/network/TileEntityPipelineBase.java:189` | TileEntityPipelineBase.getRenderBoundingBox returns INFINITE_EXTENT_AABB | open |
| B-106 | leak / perf | networks | `src/main/java/com/hbm/tileentity/network/TileEntityRequestNetwork.java:80-89` | TileEntityRequestNetwork rescans hasPath for every known node each pass | open |
| B-108 | leak / perf | networks | `src/main/java/com/hbm/uninos/networkproviders/KlystronNetwork.java:8` | Klystron/Plasma/Foundry/Rebar nets have empty update() and never purge members | open |
| B-211 | leak / perf | hazards | `src/main/java/com/hbm/handler/radiation/ChunkRadiationHandlerPRISM.java:92` | PRISM.setRadiation force-loads chunks | open |
| B-239 | leak / perf | space | `src/main/java/com/hbm/dim/eve/WorldGeneratorEve.java:56-63` | Eve volcano/spike generators cascade chunks | open |
| B-329 | leak / perf | oc | `src/main/java/com/hbm/handler/CompatHandler.java:249-252` | OCComponent extends ManagedPeripheral, disabling OC's callback cache | open |
| B-013 | dead code | core | `src/main/java/com/hbm/handler/BlockMigrations.java:14-36` | BlockMigrations.buildNumber() is never called so chunks are stamped -1 | open |
| B-016 | dead code | core | `src/main/java/com/hbm/main/ModEventHandler.java:840-858` | ModEventHandler.worldTick keeps a dead reflective `reference` field hack | open |
| B-047 | dead code | blocks-generic | `src/main/java/com/hbm/blocks/generic/BlockHazard.java:162-163` | BlockHazard.getRarity lists block_schraranium twice | open |
| B-064 | dead code | te-machine | `src/main/java/com/hbm/tileentity/machine/TileEntityMachineIGenerator.java:85-210` | machine_industrial_generator sits in the creative tab but is fully inert | open |
| B-086 | dead code | te-special | `src/main/java/com/hbm/tileentity/bomb/TileEntityLaunchPadRocket.java:235-236` | TileEntityLaunchPadRocket.isPadObstructed loops over a single block | open |
| B-115 | dead code | items | `src/main/java/com/hbm/items/ModItems.java:3459-3461` | rbmk_pellet_lecf/mecf/hecf constructed but never registered | open |
| B-116 | dead code | items | `src/main/java/com/hbm/items/ModItems.java:4050-4771` | med_schizophrenia, pch, ammo_misc instantiated but never registered | open |
| B-189 | dead code | entities | `src/main/java/com/hbm/config/BombConfig.java:97-98` | BombConfig.explosionAlgorithm and enableChunkLoading are dead options | open |
| B-213 | dead code | hazards | `src/main/java/com/hbm/config/RadiationConfig.java:72-84` | RadiationConfig.disableFibrosis and smokeStackSootMult are dead options | open |
| B-216 | dead code | hazards | `src/main/java/com/hbm/handler/radiation/ChunkRadiationHandlerNT.java:1` | Dead radiation backends NT/3D/Blank | open |
| B-242 | dead code | space | `src/main/java/com/hbm/dim/duna/genlayer/GenLayerDunaHills.java:1` | GenLayerDunaHills and GenLayerDunaPolarHills are unused | open |
| B-244 | dead code | space | `src/main/java/com/hbm/world/biome/BiomeGenNoMansLand.java:24` | BiomeGenNoMansLand and WorldProviderTom are dead and hard-code biome id 99 | open |
| B-259 | dead code | render | `src/main/java/com/hbm/particle/psys/engine/EventHandlerParticleEngine.java:14-30` | Unfinished particle psys engine is registered and allocates per world load | open |
| B-264 | dead code | render | `src/main/java/com/hbm/render/world/RenderNTMSkyboxChainloader.java:17-49` | RenderNTMSkyboxChainloader/RenderNTMSkyboxImpact are unreferenced | open |
| B-283 | dead code | net-util | `src/main/java/com/hbm/util/BobMathUtil.java:87,263` | BobMathUtil.getAngleFrom2DVecs uses cos instead of acos; setPi rewrites Math.PI | open |
| B-284 | dead code | net-util | `src/main/java/com/hbm/util/FogMessage.java:1` | FogMessage, FauxWorld and TimeAnalyzer are unused dead classes | open |
| B-299 | dead code | resources-build | `src/main/resources/assets/hbm/models/missileDoomsday.obj` | 35 OBJ models are not referenced by any Java path | open |
| B-304 | dead code | resources-build | `src/main/resources/assets/hbm/sounds/misc/desktop.ini` | Stray non-OGG and 32 unreferenced OGG files ship in sounds/ | open |

## Leave (intentional upstream design or harmless)

| ID | Severity | Area | Location | Title | Status |
|---|---|---|---|---|---|
| B-017 | crash | api | `src/main/java/api/hbm/fluidmk2/FluidNetMK2.java:24-25,45-49` | FluidNetMK2 pressure arrays are fixed at HIGHEST_VALID_PRESSURE+1 = 6 | open |
| B-038 | crash | blocks-generic | `src/main/java/com/hbm/items/block/ItemModSlab.java:14-31` | ItemModSlab hard-codes slab pairs; unknown slab yields null ItemSlab refs | open |
| B-194 | crash | hazards | `src/main/java/com/hbm/handler/atmosphere/ChunkAtmosphereHandler.java:243-256` | ChunkAtmosphereHandler register/unregisterAtmosphere lack a null guard | open |
| B-247 | crash | render | `src/main/java/com/hbm/render/anim/AnimationLoader.java:40-45` | AnimationLoader: missing JSON returns null, malformed JSON kills client | open |
| B-248 | crash | render | `src/main/java/com/hbm/render/block/RenderISBRHUniversal.java:12-17` | RenderISBRHUniversal casts block unconditionally | open |
| B-250 | crash | render | `src/main/java/com/hbm/render/loader/HFRWavefrontObject.java:177` | Mixed-mode HFRWavefrontObject throws from renderAll/renderPart | open |
| B-251 | crash | render | `src/main/java/com/hbm/render/loader/HFRWavefrontObjectVBO.java:39-47` | HFRWavefrontObjectVBO crashes client startup on quad or normal-less OBJ | open |
| B-252 | crash | render | `src/main/java/com/hbm/render/util/ObjUtil.java:58-130` | ObjUtil.renderWithIcon NPEs on OBJ without UV coordinates | open |
| B-018 | gameplay | api | `src/main/java/api/hbm/energymk2/IEnergyProviderMK2.java:43-52` | IEnergyProviderMK2.tryProvide pushes directly to any receiver, conductor or not | open |
| B-039 | gameplay | blocks-generic | `src/main/java/com/hbm/blocks/fluid/CoriumFinite.java:56-64` | CoriumFinite.updateTick deletes the block on ClassCastException | open |
| B-057 | gameplay | te-machine | `src/main/java/com/hbm/tileentity/TileEntityLoadedBase.java:95-101` | networkPackNT dedupe without getDescriptionPacket delays first sync up to 1 s | open |
| B-058 | gameplay | te-machine | `src/main/java/com/hbm/tileentity/machine/TileEntityMachineElectricFurnace.java:180` | Electric furnace can take ~60 ticks to see a new cable | open |
| B-075 | gameplay | te-special | `src/main/java/com/hbm/tileentity/bomb/TileEntityNukeBoy.java:129-141` | Assembly nukes expose no sided slots and have an odd canExtractItem | open |
| B-097 | gameplay | networks | `src/main/java/com/hbm/uninos/UniNodespace.java:151` | UniNodeWorld.popNode destroys the entire NodeNet of the removed node | open |
| B-109 | gameplay | items | `src/main/java/com/hbm/main/ModEventHandler.java:764-770` | IEquipReceiver.onEquip fires only when the held Item type changes | open |
| B-122 | gameplay | weapons | `src/main/java/com/hbm/items/weapon/sedna/mags/MagazineSingleTypeBase.java:84-91` | NPC-held sedna guns reload for free and skip EQUIP | open |
| B-123 | gameplay | weapons | `src/main/java/com/hbm/items/weapon/sedna/mods/WeaponModCaliber.java:16-39` | WeaponModCaliber returns shared static DUMMY_* magazines mutated per eval | open |
| B-138 | gameplay | recipes | `src/main/java/com/hbm/inventory/RecipesCommon.java:198-218` | ComparableStack equals accepts WILDCARD but hashCode includes meta | open |
| B-154 | gameplay | gui | `src/main/java/com/hbm/handler/nei/CustomMachineHandler.java:28` | CustomMachineHandler lacks ICompatNHNEI, invisible to GTNH NEI catalysts | open |
| B-180 | gameplay | entities | `src/main/java/com/hbm/entity/logic/EntityNukeExplosionMK5.java:63-65` | MK5 explosion grants the Manhattan achievement to every player in the world | open |
| B-181 | gameplay | entities | `src/main/java/com/hbm/entity/logic/EntityNukeExplosionMK5.java:68-80` | MK5 radiate() runs on ticks 1-9, not the first 10 | open |
| B-184 | gameplay | entities | `src/main/java/com/hbm/explosion/vanillant/ExplosionVNT.java:123-141` | ExplosionVNT.makeStandard/makeAmat use the deprecated EntityProcessorStandard | open |
| B-196 | gameplay | hazards | `src/main/java/com/hbm/handler/ae2/MassStorageMEInventory.java:34-53` | AE2 storage handlers truncate long stack sizes to int | open |
| B-219 | gameplay | space | `src/main/java/com/hbm/dim/ChunkProviderCelestial.java:414` | ChunkProviderCelestial.populate clears isHellWorld and never restores it | open |
| B-220 | gameplay | space | `src/main/java/com/hbm/dim/WorldProviderCelestial.java:693-698` | WorldProviderCelestial.canRespawnHere depends on a global static flag | open |
| B-268 | gameplay | net-util | `src/main/java/com/hbm/packet/PermaSyncHandler.java:240-247` | PermaSyncHandler CBT section aborts the whole packet on any exception | open |
| B-274 | gameplay | net-util | `src/main/java/com/hbm/wiaj/JarScript.java:146,148` | JarScript.ffwTarget/freeRun are static (one fast-forward at a time) | open |
| B-007 | silent misconfig | core | `src/main/java/com/hbm/config/GeneralConfig.java:28,118` | GeneralConfig.enableVirus field default (true) contradicts config default | open |
| B-010 | silent misconfig | core | `src/main/java/com/hbm/main/MainRegistry.java:254,682` | Two FMLPreInitializationEvent handlers (PreLoad, preInit) dispatched unordered | open |
| B-011 | silent misconfig | core | `src/main/java/com/hbm/main/MainRegistry.java:596` | ClientConfig.initConfig runs on dedicated servers, writing hbmClient.json | open |
| B-043 | silent misconfig | blocks-generic | `src/main/java/com/hbm/blocks/gas/BlockGasBase.java:102` | BlockGasBase.updateTick clears world-global scheduledUpdatesAreImmediate | open |
| B-082 | silent misconfig | te-special | `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityRBMKBase.java:61` | RBMK explodeOnBroken is a static toggled through instance references | open |
| B-111 | silent misconfig | items | `src/main/java/com/hbm/items/ModItemsArmor.java:318-330` | Two ArmorMaterials both named HBM_TRENCH via EnumHelper | open |
| B-112 | silent misconfig | items | `src/main/java/com/hbm/items/armor/ArmorFSB.java:165-184` | ArmorFSB.cloneStats shares effects list, skips hazardClass/hides/overlay | open |
| B-126 | silent misconfig | weapons | `src/main/java/com/hbm/items/weapon/sedna/mods/WeaponModBase.java:13` | Duplicate weapon-mod ids silently overwrite in HashBiMap | open |
| B-139 | silent misconfig | recipes | `src/main/java/com/hbm/config/GeneralConfig.java:47-49` | A user hbmPrecisionAssembly.json silently disables expensive mode | open |
| B-140 | silent misconfig | recipes | `src/main/java/com/hbm/config/ItemPoolConfigJSON.java:40-44` | ItemPoolConfigJSON never refreshes its template and fails silently | open |
| B-142 | silent misconfig | recipes | `src/main/java/com/hbm/inventory/fluid/Fluids.java:1130-1146` | Foreign (CompatFluidRegistry) fluids excluded from trait template/override | open |
| B-143 | silent misconfig | recipes | `src/main/java/com/hbm/inventory/fluid/Fluids.java:1181-1184` | Fluids.register(fluid, id) overwrites idMapping without a collision check | open |
| B-144 | silent misconfig | recipes | `src/main/java/com/hbm/inventory/recipes/ArcFurnaceRecipes.java:139-140` | ArcFurnaceRecipes.register silently drops recipes whose input is occupied | open |
| B-146 | silent misconfig | recipes | `src/main/java/com/hbm/inventory/recipes/loader/GenericRecipes.java:102` | nameToRecipeGlobal silently overwrites same-named recipes across sets | open |
| B-148 | silent misconfig | recipes | `src/main/java/com/hbm/inventory/recipes/loader/SerializableRecipe.java:150-153` | IRecipeRegisterListener (fork recipes) skipped when a user JSON exists | open |
| B-149 | silent misconfig | recipes | `src/main/java/com/hbm/inventory/recipes/loader/SerializableRecipe.java:244-245` | writeTemplateFile swallows every exception, leaving truncated templates | open |
| B-171 | silent misconfig | gui | `src/main/java/com/hbm/handler/imc/IMCHandlerNHNEI.java:42-45` | IMCHandlerNHNEI hard-codes panel size for every handler | open |
| B-185 | silent misconfig | entities | `src/main/java/com/hbm/entity/logic/EntityExplosionChunkloading.java:38-44` | EntityExplosionChunkloading.loadChunk is a silent one-shot | open |
| B-203 | silent misconfig | hazards | `src/main/java/com/hbm/handler/HazmatRegistry.java:233-237` | HazmatRegistry user JSON replaces the whole resistance table | open |
| B-205 | silent misconfig | hazards | `src/main/java/com/hbm/handler/radiation/ChunkRadiationHandlerSimple.java:51` | Simple backend silently drops radiation writes to unloaded chunks | open |
| B-207 | silent misconfig | hazards | `src/main/java/com/hbm/hazard/type/HazardTypeAutism.java:24` | HazardTypeAutism/Glitch are gated by disableBlinding | open |
| B-208 | silent misconfig | hazards | `src/main/java/com/hbm/inventory/OreDictManager.java:135` | OreDictManager names uranium 'Uraninite' when GT6 is loaded | open |
| B-228 | silent misconfig | space | `src/main/java/com/hbm/config/SpaceConfig.java:114-150` | Duplicate config key prefixes in SpaceConfig and WorldConfig | open |
| B-232 | silent misconfig | space | `src/main/java/com/hbm/dim/CelestialBody.java:506-514` | CelestialBody.getBody silently falls back to Kerbin | open |
| B-235 | silent misconfig | space | `src/main/java/com/hbm/dim/SolarSystemWorldSavedData.java:40` | SolarSystemWorldSavedData.get() relies on DimensionManager.getWorlds()[0] | open |
| B-324 | silent misconfig | oc | `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityCraneConsole.java:455-470` | CraneConsole getDepletion/getXenonPoison return the string 'N/A' | open |
| B-325 | silent misconfig | oc | `src/main/java/com/hbm/tileentity/machine/storage/TileEntityBatteryBase.java:222-233` | setModeLow/High narrow Lua numbers to short silently | open |
| B-102 | leak / perf | networks | `src/main/java/com/hbm/tileentity/network/RTTYSystem.java:21,61` | RTTYSystem.broadcast channels are never evicted | open |
| B-127 | leak / perf | weapons | `src/main/java/com/hbm/items/weapon/sedna/ItemGunBaseNT.java:473-474` | ItemGunBaseNT.getShareTag disabled: full gun NBT resynced on every change | open |
| B-188 | leak / perf | entities | `src/main/java/com/hbm/explosion/ExplosionNukeRayBatched.java:260-264` | ExplosionNukeRayBatched.cacheChunksTick ignores its time budget | open |
| B-210 | leak / perf | hazards | `src/main/java/com/hbm/handler/neutron/NeutronNodeWorld.java:39-43` | NeutronNodeWorld.removeEmptyWorlds drops node caches when a world has no streams | open |
| B-240 | leak / perf | space | `src/main/java/com/hbm/main/ModEventHandler.java:823-831` | overrideOverworldProvider re-registers provider 0 on every non-Earth world load | open |
| B-277 | leak / perf | net-util | `src/main/java/com/hbm/blocks/BlockVolcanoV2.java:91` | BufPacket is also sent directly, bypassing networkPackNT's dedupe and threading | open |
| B-012 | dead code | core | `src/main/java/com/hbm/extprop/HbmPlayerProps.java:225-227` | HbmPlayerProps/HbmLivingProps NBT persistence methods are marked @Deprecated | open |
| B-014 | dead code | core | `src/main/java/com/hbm/handler/HbmKeybinds.java:40,67,229` | EnumKeybind.SLAM / slamKey is registered but has no handler or proxy case | open |
| B-015 | dead code | core | `src/main/java/com/hbm/main/MainRegistry.java:674,748` | CommandWikiRender is registered both as a client and as a server command | open |
| B-031 | dead code | api | `src/main/java/api/hbm/block/IRadioControllable.java:1` | Dead API interfaces with zero implementers; two lack @Deprecated | open |
| B-032 | dead code | api | `src/main/java/api/hbm/energymk2/Nodespace.java:24-33` | @Deprecated Nodespace statics are still the API's own subscription path | open |
| B-033 | dead code | api | `src/main/java/api/hbm/entity/RadarEntry.java:47-65` | RadarEntry.fromBytes/toBytes omit the redstone field | open |
| B-035 | dead code | api | `src/main/java/api/hbm/ntl/StackCache.java:181` | StackCache.CacheSlot.reCount is self-documented as unused | open |
| B-036 | dead code | api | `src/main/java/cofh/api/energy/TileEnergyHandler.java:1` | Vendored cofh TileEnergyHandler and ItemEnergyContainer are unused | open |
| B-045 | dead code | blocks-generic | `src/main/java/com/hbm/blocks/BlockBase.java:36-41` | BlockBase.setBeaconable()/noMobSpawn() have no call sites | open |
| B-046 | dead code | blocks-generic | `src/main/java/com/hbm/blocks/ModBlocks.java:4052-4055` | Block rename compat path is dead: addRemap unused, no BLOCK remap branch | open |
| B-048 | dead code | blocks-generic | `src/main/java/com/hbm/blocks/turret/TurretBase.java:1` | blocks/turret/TurretBase.java is dead (zero subclasses) | open |
| B-062 | dead code | te-machine | `src/main/java/com/hbm/tileentity/IRTGUser.java:83-153` | IRTGUser interface (with three @Untested overloads) has no implementers | open |
| B-063 | dead code | te-machine | `src/main/java/com/hbm/tileentity/TileEntityMachineBase.java:140` | TileEntityMachineBase.getGaugeScaled takes Forge's FluidTank (dead, misleading) | open |
| B-065 | dead code | te-machine | `src/main/java/com/hbm/tileentity/machine/TileEntityMachineWarController.java:40-60` | TileEntityMachineWarController is an auto-generated stub (isLoaded() false) | open |
| B-087 | dead code | te-special | `src/main/java/com/hbm/tileentity/bomb/TileEntityNukeBoy.java:185` | TileEntityNukeBoy.isReady uses octal literal slots[01] | open |
| B-088 | dead code | te-special | `src/main/java/com/hbm/tileentity/bomb/TileEntityNukeCustom.java:385` | TileEntityNukeCustom.destruct() passes xCoord as z (and is never called) | open |
| B-089 | dead code | te-special | `src/main/java/com/hbm/tileentity/machine/TileEntityPWRController.java:557` | TileEntityPWRController writes "rodCount" twice in writeToNBT | open |
| B-090 | dead code | te-special | `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityRBMKBase.java:602-604` | TileEntityRBMKBase.getFancyStats(NBT) is a static stub returning null | open |
| B-091 | dead code | te-special | `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityRBMKConsole.java:66-68` | TileEntityRBMKConsole.getName() returns null | open |
| B-092 | dead code | te-special | `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityRBMKRod.java:134` | ItemRBMKRod.specialFluxCurve is never enabled; rod branch is dead | open |
| B-118 | dead code | items | `src/main/resources/assets/hbm/textures/items/achievement_icon.digammaunity.png:1` | Orphan texture achievement_icon.digammaunity.png | open |
| B-128 | dead code | weapons | `src/main/java/com/hbm/entity/missile/EntityMissileCustom.java:179-243` | EntityMissileCustom impact switch: CLUSTER no-op, several types unhandled | open |
| B-129 | dead code | weapons | `src/main/java/com/hbm/entity/projectile/EntityBullet.java:408-416` | EntityBullet legacy 'test feature' left in release code | open |
| B-130 | dead code | weapons | `src/main/java/com/hbm/entity/projectile/EntityBulletBeamBase.java:325` | EntityBulletBeamBase kills itself on NBT read | open |
| B-131 | dead code | weapons | `src/main/java/com/hbm/handler/guncfg/GunDGKFactory.java:8-20` | GunDGKFactory is dead code (CASINGDGK never referenced) | open |
| B-132 | dead code | weapons | `src/main/java/com/hbm/items/weapon/sedna/ItemGunBaseNT.java:276` | playAnimation always sends receiverIndex 0 to the recoil LambdaContext | open |
| B-133 | dead code | weapons | `src/main/java/com/hbm/items/weapon/sedna/factory/XFactory35800.java:56-58` | SpentCasing "35-800" registered twice with identical stats | open |
| B-152 | dead code | recipes | `src/main/java/com/hbm/inventory/recipes/loader/SerializableRecipe.java:85-95` | Stale '//AFTER Assembler' ordering comment in registerAllHandlers | open |
| B-173 | dead code | gui | `src/main/java/com/hbm/handler/nei/AlloyFurnaceRecipeHandler.java:1` | AlloyFurnaceRecipeHandler is never registered | open |
| B-174 | dead code | gui | `src/main/java/com/hbm/inventory/container/ContainerNT.java:1` | ContainerNT is dead code | open |
| B-175 | dead code | gui | `src/main/java/com/hbm/inventory/gui/GUIMachineReactorBreeding.java:53-63` | GUIMachineReactorBreeding carries a stale 'dud TE' comment with no handling | open |
| B-190 | dead code | entities | `src/main/java/com/hbm/entity/ModEntityList.java:62-124` | ModEntityList lookup helpers are dead code | open |
| B-192 | dead code | entities | `src/main/java/com/hbm/explosion/ExplosionNukeRayBalefire.java:1` | ExplosionNukeRayBalefire is never instantiated | open |
| B-215 | dead code | hazards | `src/main/java/com/hbm/handler/pollution/PollutionHandler.java:358` | PollutionType.FALLOUT is an unused enum slot | open |
| B-245 | dead code | space | `src/main/java/com/hbm/world/feature/OreLayer.java:1` | OreLayer (2D) is never instantiated | open |
| B-246 | dead code | space | `src/main/java/com/hbm/world/test/WorldGenTest.java:1` | world/test scaffolding is unreferenced | open |
| B-260 | dead code | render | `src/main/java/com/hbm/render/entity/projectile/RenderBombletTheta.java:22` | RenderBombletTheta fallback texture bombletThetaTexture.png missing | open |
| B-263 | dead code | render | `src/main/java/com/hbm/render/tileentity/RendererObjTester.java:436-445` | Test renderers reference missing textures (ObjTester, MinecartTest) | open |
| B-280 | dead code | net-util | `src/main/java/com/hbm/packet/GuiLayerPacket.java:38` | GuiLayerPacket handler has an unreachable client-side check | open |
| B-281 | dead code | net-util | `src/main/java/com/hbm/packet/PacketDispatcher.java:76-77` | ContainerNBTCommsPacket is registered twice (discriminators 27 and 28) | open |
| B-282 | dead code | net-util | `src/main/java/com/hbm/packet/toserver/AuxButtonPacket.java:29,150` | AuxButtonPacket is @Deprecated yet used by 19 GUIs; value 999 spawns a duck | open |
| B-298 | dead code | resources-build | `src/main/resources/assets/hbm/lang/en_US.lang:5577` | Lang key soundCategory.ntmMachines has no implementation | open |
| B-301 | dead code | resources-build | `src/main/resources/assets/hbm/my_hecking_realism.png` | Unreferenced images my_hecking_realism.png and textures/ABC123.png | open |
| B-302 | dead code | resources-build | `src/main/resources/assets/hbm/shaders/supernovae.frag` | Shaders blackholed.frag and supernovae.frag are unreferenced | open |
| B-305 | dead code | resources-build | `src/main/resources/assets/hbm/structures/crane.nbt` | Seven unreferenced structure NBTs ship in the jar | open |
| B-330 | dead code | oc | `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityCraneConsole.java:400-412` | worldObj.isRemote checks inside OC callbacks are dead code | open |

## Upstream-only (report, do not carry)

| ID | Severity | Area | Location | Title | Status |
|---|---|---|---|---|---|
| B-249 | crash | render | `src/main/java/com/hbm/render/icon/TextureAtlasSpriteMutatable.java:24-25` | TextureAtlasSpriteMutatable crashes stitching when base texture is missing | open |
| B-071 | gameplay | te-special | `src/main/java/com/hbm/handler/neutron/RBMKNeutronHandler.java:385` | RBMK stream tail check (#1933) tests the origin node, so the branch is dead | open |
| B-119 | gameplay | weapons | `src/main/java/com/hbm/entity/projectile/EntityArtilleryRocket.java:34-35` | EntityArtilleryRocket keeps a raw Entity target reference | open |
| B-124 | gameplay | weapons | `src/main/java/com/hbm/items/weapon/sedna/mods/XWeaponModManager.java:284-296` | restoreMagState only restores receiver 0 after mod changes | open |
| B-254 | gameplay | render | `src/main/java/com/hbm/main/ModEventHandlerRenderer.java:673-676` | Badge HUD still hidden by F1 despite ElementType.ALL | open |
| B-255 | gameplay | render | `src/main/java/com/hbm/render/anim/BusAnimationSequence.java:83-87` | BusAnimationSequence.holdUntil wrong when animation speed is not 1 | open |
| B-276 | silent misconfig | net-util | `src/main/java/com/hbm/util/CompatRecipeRegistry.java:288-292` | CompatRecipeRegistry deprecated registerAssembler/registerChemplant are NOPs | open |
| B-084 | leak / perf | te-special | `src/main/java/com/hbm/handler/neutron/NeutronNodeWorld.java:70-80` | NeutronNodeWorld.cleanNodes never uncaches PILE nodes | open |
| B-114 | dead code | items | `src/main/java/com/hbm/items/ModItems.java:249-2232` | Six ModItems fields declared but never assigned (null) | open |
| B-117 | dead code | items | `src/main/java/com/hbm/items/ModItemsArmor.java:276-277` | jackt/jackt2 instantiated twice in ModItemsArmor.init() | open |
| B-285 | dead code | net-util | `src/main/java/com/hbm/wiaj/actors/ActorFancyPanel.java:130-133,148-151,340` | ActorFancyPanel measures scaled-stack elements but never draws them | open |
...
- **B-006** · silent misconfig · fix in the fork · `src/main/java/com/hbm/config/FalloutConfigJSON.java:292`
  ...
  - Status: open
- **B-007** · silent misconfig · leave · `src/main/java/com/hbm/config/GeneralConfig.java:28,118`
...
- **B-009** · silent misconfig · fix in the fork · `src/main/java/com/hbm/handler/Identity.java:35`
  ...
  - Status: open
- **B-010** · silent misconfig · leave · `src/main/java/com/hbm/main/MainRegistry.java:254,682`
  ...
  - Status: open
- **B-011** · silent misconfig · leave · `src/main/java/com/hbm/main/MainRegistry.java:596`
  ...
  - Status: open
- **B-012** · dead code · leave · `src/main/java/com/hbm/extprop/HbmPlayerProps.java:225-227`
...
- **B-013** · dead code · needs a decision · `src/main/java/com/hbm/handler/BlockMigrations.java:14-36`
  ...
  - Status: open
- **B-014** · dead code · leave · `src/main/java/com/hbm/handler/HbmKeybinds.java:40,67,229`
  ...
  - Status: open
- **B-015** · dead code · leave · `src/main/java/com/hbm/main/MainRegistry.java:674,748`
...
- **B-017** · crash · leave · `src/main/java/api/hbm/fluidmk2/FluidNetMK2.java:24-25,45-49`
  ...
  - Status: open
- **B-018** · gameplay · leave · `src/main/java/api/hbm/energymk2/IEnergyProviderMK2.java:43-52`
...
- **B-030** · leak / perf · needs a decision · `src/main/java/api/hbm/tile/ILoadedTile.java:20,48`
  ...
  - Status: open
- **B-031** · dead code · leave · `src/main/java/api/hbm/block/IRadioControllable.java:1`
  ...
  - Status: open
- **B-032** · dead code · leave · `src/main/java/api/hbm/energymk2/Nodespace.java:24-33`
...
- **B-034** · dead code · fix in the fork · `src/main/java/api/hbm/fluidmk2/FluidNetMK2.java:49`
  ...
  - Status: open
- **B-035** · dead code · leave · `src/main/java/api/hbm/ntl/StackCache.java:181`
...
- **B-037** · crash · fix in the fork · `src/main/java/com/hbm/blocks/fluid/CoriumFluid.java:23-29`
  ...
  - Status: open
- **B-038** · crash · leave · `src/main/java/com/hbm/items/block/ItemModSlab.java:14-31`
  ...
  - Status: open
- **B-039** · gameplay · leave · `src/main/java/com/hbm/blocks/fluid/CoriumFinite.java:56-64`
...
- **B-042** · silent misconfig · needs a decision · `src/main/java/com/hbm/blocks/ModBlocks.java:3110`
  ...
  - Status: open
- **B-043** · silent misconfig · leave · `src/main/java/com/hbm/blocks/gas/BlockGasBase.java:102`
...
- **B-044** · silent misconfig · needs a decision · `src/main/java/com/hbm/main/ModEventHandlerClient.java:225`
  ...
  - Status: open
- **B-045** · dead code · leave · `src/main/java/com/hbm/blocks/BlockBase.java:36-41`
...
- **B-047** · dead code · needs a decision · `src/main/java/com/hbm/blocks/generic/BlockHazard.java:162-163`
  ...
  - Status: open
- **B-048** · dead code · leave · `src/main/java/com/hbm/blocks/turret/TurretBase.java:1`
...
- **B-056** · gameplay · fix in the fork · `src/main/java/com/hbm/inventory/UpgradeManagerNT.java:57-66`
  ...
  - Status: open
- **B-057** · gameplay · leave · `src/main/java/com/hbm/tileentity/TileEntityLoadedBase.java:95-101`
  ...
  - Status: open
- **B-058** · gameplay · leave · `src/main/java/com/hbm/tileentity/machine/TileEntityMachineElectricFurnace.java:180`
...
- **B-061** · leak / perf · fix in the fork · `src/main/java/com/hbm/tileentity/TileEntityLoadedBase.java:36-39`
  ...
  - Status: open
- **B-062** · dead code · leave · `src/main/java/com/hbm/tileentity/IRTGUser.java:83-153`
  ...
  - Status: open
- **B-063** · dead code · leave · `src/main/java/com/hbm/tileentity/TileEntityMachineBase.java:140`
...
- **B-070** · gameplay · needs a decision · `src/main/java/com/hbm/handler/neutron/RBMKNeutronHandler.java:286-290`
  ...
  - Status: open
- **B-071** · gameplay · upstream-only · `src/main/java/com/hbm/handler/neutron/RBMKNeutronHandler.java:385`
...
- **B-083** · silent misconfig · fix in the fork · `src/main/java/com/hbm/util/GameRuleHelper.java:29-30`
  ...
  - Status: open
- **B-084** · leak / perf · upstream-only · `src/main/java/com/hbm/handler/neutron/NeutronNodeWorld.java:70-80`
...
- **B-086** · dead code · needs a decision · `src/main/java/com/hbm/tileentity/bomb/TileEntityLaunchPadRocket.java:235-236`
  ...
  - Status: open
- **B-087** · dead code · leave · `src/main/java/com/hbm/tileentity/bomb/TileEntityNukeBoy.java:185`
  ...
  - Fix: Write slots[1].
  - Status: open
- **B-088** · dead code · leave · `src/main/java/com/hbm/tileentity/bomb/TileEntityNukeCustom.java:385`
  ...
  - Status: open
- **B-089** · dead code · leave · `src/main/java/com/hbm/tileentity/machine/TileEntityPWRController.java:557`
  ...
  - Status: open
- **B-090** · dead code · leave · `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityRBMKBase.java:602-604`
  ...
  - Fix: Delete the stub.
  - Status: open
- **B-091** · dead code · leave · `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityRBMKConsole.java:66-68`
...
- **B-101** · leak / perf · fix in the fork · `src/main/java/com/hbm/tileentity/machine/storage/TileEntityBatteryBase.java:65,102,111`
  ...
  - Status: open
- **B-102** · leak / perf · leave · `src/main/java/com/hbm/tileentity/network/RTTYSystem.java:21,61`
...
- **B-109** · gameplay · leave · `src/main/java/com/hbm/main/ModEventHandler.java:764-770`
...
- **B-110** · gameplay · fix in the fork · `src/main/java/com/hbm/main/ModEventHandlerClient.java:791`
  ...
  - Status: open
- **B-111** · silent misconfig · leave · `src/main/java/com/hbm/items/ModItemsArmor.java:318-330`
  ...
  - Status: open
- **B-112** · silent misconfig · leave · `src/main/java/com/hbm/items/armor/ArmorFSB.java:165-184`
...
- **B-113** · silent misconfig · needs a decision · `src/main/java/com/hbm/items/food/ItemLemon.java:40`
  ...
  - Status: open
- **B-114** · dead code · upstream-only · `src/main/java/com/hbm/items/ModItems.java:249-2232`
...
- **B-116** · dead code · needs a decision · `src/main/java/com/hbm/items/ModItems.java:4050-4771`
  ...
  - Status: open
- **B-117** · dead code · upstream-only · `src/main/java/com/hbm/items/ModItemsArmor.java:276-277`
  ...
  - Status: open
- **B-118** · dead code · leave · `src/main/resources/assets/hbm/textures/items/achievement_icon.digammaunity.png:1`
...
- **B-119** · gameplay · upstream-only · `src/main/java/com/hbm/entity/projectile/EntityArtilleryRocket.java:34-35`
...
- **B-122** · gameplay · leave · `src/main/java/com/hbm/items/weapon/sedna/mags/MagazineSingleTypeBase.java:84-91`
  ...
  - Status: open
- **B-123** · gameplay · leave · `src/main/java/com/hbm/items/weapon/sedna/mods/WeaponModCaliber.java:16-39`
  ...
  - Status: open
- **B-124** · gameplay · upstream-only · `src/main/java/com/hbm/items/weapon/sedna/mods/XWeaponModManager.java:284-296`
...
- **B-125** · gameplay · fix in the fork · `src/main/java/com/hbm/items/weapon/sedna/mods/XWeaponModManager.java:308`
  ...
  - Status: open
- **B-126** · silent misconfig · leave · `src/main/java/com/hbm/items/weapon/sedna/mods/WeaponModBase.java:13`
...
- **B-128** · dead code · leave · `src/main/java/com/hbm/entity/missile/EntityMissileCustom.java:179-243`
  ...
  - Status: open
- **B-129** · dead code · leave · `src/main/java/com/hbm/entity/projectile/EntityBullet.java:408-416`
...
- **B-130** · dead code · leave · `src/main/java/com/hbm/entity/projectile/EntityBulletBeamBase.java:325`
  ...
  - Fix: None (design).
  - Status: open
- **B-131** · dead code · leave · `src/main/java/com/hbm/handler/guncfg/GunDGKFactory.java:8-20`
  ...
  - Status: open
- **B-132** · dead code · leave · `src/main/java/com/hbm/items/weapon/sedna/ItemGunBaseNT.java:276`
  ...
  - Status: open
- **B-133** · dead code · leave · `src/main/java/com/hbm/items/weapon/sedna/factory/XFactory35800.java:56-58`
...
- **B-138** · gameplay · leave · `src/main/java/com/hbm/inventory/RecipesCommon.java:198-218`
  ...
  - Status: open
- **B-139** · silent misconfig · leave · `src/main/java/com/hbm/config/GeneralConfig.java:47-49`
  ...
  - Status: open
- **B-140** · silent misconfig · leave · `src/main/java/com/hbm/config/ItemPoolConfigJSON.java:40-44`
...
- **B-141** · silent misconfig · fix in the fork · `src/main/java/com/hbm/inventory/fluid/Fluids.java:1085-1099`
  ...
  - Status: open
- **B-142** · silent misconfig · leave · `src/main/java/com/hbm/inventory/fluid/Fluids.java:1130-1146`
  ...
  - Status: open
- **B-143** · silent misconfig · leave · `src/main/java/com/hbm/inventory/fluid/Fluids.java:1181-1184`
  ...
  - Status: open
- **B-144** · silent misconfig · leave · `src/main/java/com/hbm/inventory/recipes/ArcFurnaceRecipes.java:139-140`
...
- **B-145** · silent misconfig · needs a decision · `src/main/java/com/hbm/inventory/recipes/AtmosphereRecipes.java:70-73`
  ...
  - Status: open
- **B-146** · silent misconfig · leave · `src/main/java/com/hbm/inventory/recipes/loader/GenericRecipes.java:102`
...
- **B-148** · silent misconfig · leave · `src/main/java/com/hbm/inventory/recipes/loader/SerializableRecipe.java:150-153`
  ...
  - Status: open
- **B-149** · silent misconfig · leave · `src/main/java/com/hbm/inventory/recipes/loader/SerializableRecipe.java:244-245`
...
- **B-151** · dead code · fix in the fork · `src/main/java/com/hbm/inventory/recipes/SILEXRecipes.java:51-58`
  ...
  - Status: open
- **B-152** · dead code · leave · `src/main/java/com/hbm/inventory/recipes/loader/SerializableRecipe.java:85-95`
...
- **B-154** · gameplay · leave · `src/main/java/com/hbm/handler/nei/CustomMachineHandler.java:28`
...
- **B-170** · gameplay · needs a decision · `src/main/java/com/hbm/main/NEIRegistry.java:14-16`
  ...
  - Status: open
- **B-171** · silent misconfig · leave · `src/main/java/com/hbm/handler/imc/IMCHandlerNHNEI.java:42-45`
...
- **B-172** · silent misconfig · fix in the fork · `src/main/java/com/hbm/handler/nei/BreederRecipeHandler.java:70-120`
  ...
  - Status: open
- **B-173** · dead code · leave · `src/main/java/com/hbm/handler/nei/AlloyFurnaceRecipeHandler.java:1`
  ...
  - Status: open
- **B-174** · dead code · leave · `src/main/java/com/hbm/inventory/container/ContainerNT.java:1`
  ...
  - Status: open
- **B-175** · dead code · leave · `src/main/java/com/hbm/inventory/gui/GUIMachineReactorBreeding.java:53-63`
...
- **B-179** · gameplay · fix in the fork · `src/main/java/com/hbm/entity/effect/EntityFalloutRain.java:38-42`
  ...
  - Status: open
- **B-180** · gameplay · leave · `src/main/java/com/hbm/entity/logic/EntityNukeExplosionMK5.java:63-65`
  ...
  - Status: open
- **B-181** · gameplay · leave · `src/main/java/com/hbm/entity/logic/EntityNukeExplosionMK5.java:68-80`
...
- **B-183** · gameplay · needs a decision · `src/main/java/com/hbm/explosion/ExplosionNukeRayBatched.java:89-98`
  ...
  - Status: open
- **B-184** · gameplay · leave · `src/main/java/com/hbm/explosion/vanillant/ExplosionVNT.java:123-141`
  ...
  - Status: open
- **B-185** · silent misconfig · leave · `src/main/java/com/hbm/entity/logic/EntityExplosionChunkloading.java:38-44`
...
- **B-187** · silent misconfig · needs a decision · `src/main/java/com/hbm/main/ClientProxy.java:796`
  ...
  - Status: open
- **B-188** · leak / perf · leave · `src/main/java/com/hbm/explosion/ExplosionNukeRayBatched.java:260-264`
  ...
  - Evidence: `// time ignored here since collectTip() did not implement a time limit
 collectTip(speed*10);`
...
- **B-189** · dead code · needs a decision · `src/main/java/com/hbm/config/BombConfig.java:97-98`
  ...
  - Status: open
- **B-190** · dead code · leave · `src/main/java/com/hbm/entity/ModEntityList.java:62-124`
...
- **B-191** · dead code · fix in the fork · `src/main/java/com/hbm/entity/train/EntityRailCarBase.java:119-133`
  ...
  - Status: open
- **B-192** · dead code · leave · `src/main/java/com/hbm/explosion/ExplosionNukeRayBalefire.java:1`
...
- **B-193** · crash · needs a decision · `src/main/java/com/hbm/handler/atmosphere/AtmosphereBlob.java:171-175`
  ...
  - Status: open
- **B-194** · crash · leave · `src/main/java/com/hbm/handler/atmosphere/ChunkAtmosphereHandler.java:243-256`
...
- **B-195** · crash · fix in the fork · `src/main/java/com/hbm/saveddata/SatelliteSavedData.java:48-49`
  ...
  - Status: open
- **B-196** · gameplay · leave · `src/main/java/com/hbm/handler/ae2/MassStorageMEInventory.java:34-53`
...
- **B-202** · silent misconfig · needs a decision · `src/main/java/com/hbm/config/RadiationConfig.java:31-73`
  ...
  - Status: open
- **B-203** · silent misconfig · leave · `src/main/java/com/hbm/handler/HazmatRegistry.java:233-237`
...
- **B-206** · silent misconfig · fix in the fork · `src/main/java/com/hbm/hazard/HazardSystem.java:62-63`
  ...
  - Status: open
- **B-207** · silent misconfig · leave · `src/main/java/com/hbm/hazard/type/HazardTypeAutism.java:24`
...
- **B-209** · silent misconfig · needs a decision · `src/main/java/com/hbm/saveddata/satellites/XSatelliteRegistry.java:35-36`
  ...
  - Status: open
- **B-210** · leak / perf · leave · `src/main/java/com/hbm/handler/neutron/NeutronNodeWorld.java:39-43`
...
- **B-218** · crash · fix in the fork · `src/main/java/com/hbm/world/feature/BedrockOre.java:56-60`
  ...
  - Status: open
- **B-219** · gameplay · leave · `src/main/java/com/hbm/dim/ChunkProviderCelestial.java:414`
  ...
  - Status: open
- **B-220** · gameplay · leave · `src/main/java/com/hbm/dim/WorldProviderCelestial.java:693-698`
...
- **B-231** · silent misconfig · needs a decision · `src/main/java/com/hbm/dim/CelestialBody.java:262-266`
  ...
  - Status: open
- **B-232** · silent misconfig · leave · `src/main/java/com/hbm/dim/CelestialBody.java:506-514`
...
- **B-234** · silent misconfig · needs a decision · `src/main/java/com/hbm/dim/SolarSystem.java:275-321`
  ...
  - Status: open
- **B-235** · silent misconfig · leave · `src/main/java/com/hbm/dim/SolarSystemWorldSavedData.java:40`
...
- **B-239** · leak / perf · needs a decision · `src/main/java/com/hbm/dim/eve/WorldGeneratorEve.java:56-63`
  ...
  - Status: open
- **B-240** · leak / perf · leave · `src/main/java/com/hbm/main/ModEventHandler.java:823-831`
...
- **B-244** · dead code · needs a decision · `src/main/java/com/hbm/world/biome/BiomeGenNoMansLand.java:24`
  ...
  - Status: open
- **B-245** · dead code · leave · `src/main/java/com/hbm/world/feature/OreLayer.java:1`
  ...
  - Status: open
- **B-246** · dead code · leave · `src/main/java/com/hbm/world/test/WorldGenTest.java:1`
...
- **B-247** · crash · leave · `src/main/java/com/hbm/render/anim/AnimationLoader.java:40-45`
  ...
  - Status: open
- **B-248** · crash · leave · `src/main/java/com/hbm/render/block/RenderISBRHUniversal.java:12-17`
  ...
  - Status: open
- **B-249** · crash · upstream-only · `src/main/java/com/hbm/render/icon/TextureAtlasSpriteMutatable.java:24-25`
...
- **B-250** · crash · leave · `src/main/java/com/hbm/render/loader/HFRWavefrontObject.java:177`
  ...
  - Status: open
- **B-251** · crash · leave · `src/main/java/com/hbm/render/loader/HFRWavefrontObjectVBO.java:39-47`
  ...
  - Status: open
- **B-252** · crash · leave · `src/main/java/com/hbm/render/util/ObjUtil.java:58-130`
...
- **B-253** · gameplay · fix in the fork · `src/main/java/com/hbm/main/ModEventHandlerClient.java:354-371`
  ...
  - Status: open
- **B-254** · gameplay · upstream-only · `src/main/java/com/hbm/main/ModEventHandlerRenderer.java:673-676`
  ...
  - Status: open
- **B-255** · gameplay · upstream-only · `src/main/java/com/hbm/render/anim/BusAnimationSequence.java:83-87`
...
- **B-259** · dead code · needs a decision · `src/main/java/com/hbm/particle/psys/engine/EventHandlerParticleEngine.java:14-30`
  ...
  - Status: open
- **B-260** · dead code · leave · `src/main/java/com/hbm/render/entity/projectile/RenderBombletTheta.java:22`
...
- **B-262** · dead code · fix in the fork · `src/main/java/com/hbm/render/tileentity/RenderVaultDoor.java:34-77`
  ...
  - Status: open
- **B-263** · dead code · leave · `src/main/java/com/hbm/render/tileentity/RendererObjTester.java:436-445`
...
- **B-267** · crash · fix in the fork · `src/main/java/com/hbm/util/BufferUtil.java:104,114`
  ...
  - Status: open
- **B-268** · gameplay · leave · `src/main/java/com/hbm/packet/PermaSyncHandler.java:240-247`
...
- **B-273** · gameplay · fix in the fork · `src/main/java/com/hbm/util/Tuple.java:236`
  ...
  - Status: open
- **B-274** · gameplay · leave · `src/main/java/com/hbm/wiaj/JarScript.java:146,148`
...
- **B-275** · silent misconfig · fix in the fork · `src/main/java/com/hbm/main/NetworkHandler.java:56-57`
  ...
  - Status: open
- **B-276** · silent misconfig · upstream-only · `src/main/java/com/hbm/util/CompatRecipeRegistry.java:288-292`
  ...
  - Status: open
- **B-277** · leak / perf · leave · `src/main/java/com/hbm/blocks/BlockVolcanoV2.java:91`
...
- **B-279** · dead code · fix in the fork · `src/main/java/com/hbm/main/NetworkHandler.java:168-183`
  ...
  - Status: open
- **B-280** · dead code · leave · `src/main/java/com/hbm/packet/GuiLayerPacket.java:38`
...
- **B-284** · dead code · needs a decision · `src/main/java/com/hbm/util/FogMessage.java:1`
  ...
  - Status: open
- **B-285** · dead code · upstream-only · `src/main/java/com/hbm/wiaj/actors/ActorFancyPanel.java:130-133,148-151,340`
...
- **B-297** · dead code · fix in the fork · `src/main/java/com/hbm/main/StructureManager.java:165`
  ...
  - Status: open
- **B-298** · dead code · leave · `src/main/resources/assets/hbm/lang/en_US.lang:5577`
...
- **B-300** · dead code · fix in the fork · `src/main/resources/assets/hbm/models/weapons/.obj`
  ...
  - Fix: Delete the file.
  - Status: open
- **B-301** · dead code · leave · `src/main/resources/assets/hbm/my_hecking_realism.png`
  ...
  - Status: open
- **B-302** · dead code · leave · `src/main/resources/assets/hbm/shaders/supernovae.frag`
...
- **B-304** · dead code · needs a decision · `src/main/resources/assets/hbm/sounds/misc/desktop.ini`
  ...
  - Status: open
- **B-305** · dead code · leave · `src/main/resources/assets/hbm/structures/crane.nbt`
...
- **B-323** · silent misconfig · fix in the fork · `src/main/java/com/hbm/tileentity/machine/TileEntityWatz.java:55`
  ...
  - Status: open
- **B-324** · silent misconfig · leave · `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityCraneConsole.java:455-470`
  ...
  - Status: open
- **B-325** · silent misconfig · leave · `src/main/java/com/hbm/tileentity/machine/storage/TileEntityBatteryBase.java:222-233`
...
- **B-329** · leak / perf · needs a decision · `src/main/java/com/hbm/handler/CompatHandler.java:249-252`
  ...
  - Status: open
- **B-330** · dead code · leave · `src/main/java/com/hbm/tileentity/machine/rbmk/TileEntityCraneConsole.java:400-412`
